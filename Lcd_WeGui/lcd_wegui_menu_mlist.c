...
void Wegui_show_mList(uint16_t farmes)
{
	...
	if(id_min==0)
	{
		...
		*/
		#if (LCD_COLOUR_BIT == 1)
			Lcd_Draw_UTF8_String	(SCREEN_WIDTH - 1 - Lcd_Get_UTF8_XLen((uint8_t*)"OLED") - 2,
															temp_y,
															(uint8_t*)"OLED");//LOGO
		#else
			Lcd_Draw_UTF8_String	(SCREEN_WIDTH - 1 - Lcd_Get_UTF8_XLen((uint8_t*)"RGB") - 2,
															temp_y,
															(uint8_t*)"RGB");//LOGO
		#endif
	...
	if(farmes != 0)
	{
		...
		Value_Change_PID_P( (mList_par.cursor_box_x1),
												(curr_target_x0+(Lcd_Get_UTF8_XLen((uint8_t*)string))),
												(1),
												(farmes));
		
		//----------------B 无动画----------------
		///mList_par.cursor_box_x1=curr_target_x0+(Lcd_Get_UTF8_XLen((uint8_t*)string));
