...
//-------------------------3.2设定硬件SPI时钟速率-------------------------------
#if ((LCD_PORT == _HARD_4SPI) || (LCD_PORT == _DMA_4SPI) || (LCD_PORT == _OTHER))

//设置硬件SPI分频
//GD32E230手册描述SPI最高设置18MHz 实测支持超频(72MHz/SPI_PSC_4=18MHz)
#define SPI_PSC_x       SPI_PSC_4 
#endif
...
//仅适配了TFT彩屏 //每页连续横向扫描DYNA_CRC_ONCE_NUM个像素校验一个crc
	...
	#define PAGE_CRC_NUM  (4)     //动态刷新每页细分校验的分区 默认1 仅TFT可用,其他类型屏幕默认取1
#endif
