...
#define STR_MCU_CLASS "GD32E2"
#define STR_MCU_MODEL "GD32E230C8"
...
//--------------------------2.选择一个GUI菜单交互方式-----------------------------
#define _DEMO_4KEY_PORT     (0) //4按键交互接口 "上","下","左","右" 对应文件gd32e230_wegui_4key_port.c
#define _DEMO_6KEY_PORT     (1) //6按键交互接口 "上","下","左","右","OK","BACK" 对应文件gd32e230_wegui_6key_port.c
#define _OTHER_PROJECT_PORT (2) //项目自定义交互接口 对应文件gd32e230_wegui_other_port.c

#define WEGUI_INTERFACE_PORT  _DEMO_6KEY_PORT //选择一个交互接口



//#if (WEGUI_INTERFACE_PORT ==_OTHER_PROJECT_PORT)
...
#if((SCREEN_WIDTH*GRAM_YPAGE_NUM*LCD_COLOUR_BIT + Stack_Size)>8192)
	//RAM不足, 请使用页缓存模式, 降低显存占用
	#error("RAM Not enough!")
#endif
...
#if (WEGUI_INTERFACE_PORT ==_DEMO_6KEY_PORT)    //6键交互模式
	#include "gd32e230_wegui_6key_port.h"
	#define Wegui_Interface_port_Init() do{Wegui_6key_port_Init();Wegui_Uart_Port_Init();}while(0)
#elif (WEGUI_INTERFACE_PORT ==_DEMO_4KEY_PORT)    //4键交互模式
	#include "gd32e230_wegui_4key_port.h"
	#define Wegui_Interface_port_Init() do{Wegui_4key_port_Init();Wegui_Uart_Port_Init();}while(0)
#elif (WEGUI_INTERFACE_PORT ==_OTHER_PROJECT_PORT) //自定义
	//#include "gd32e230_wegui_project_port.h"
	#define Wegui_Interface_port_Init() do{/*Wegui_project_interface_port_Init();*/}while(0)
	#define Wegui_Interface_stick(ms) do{/*Wegui_project_Interface_stick(ms);*/}while(0)
#else
	#warning("need interface.c")
